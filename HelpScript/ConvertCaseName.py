                    ...
                    if action == 1:
                        if i < 9:
                            if "Windows" in platform.platform():
                                os.rename("%s\\%s" % (json, dirs[i]), "%s\\%s" % (json, ("00%d_" % (i + 1) + dirs[i])))
                            else:
                                os.rename("%s/%s" % (json, dirs[i]), "%s/%s" % (json, ("00%d_" % (i+1)+dirs[i])))
                        if 9 <= i < 99:
                            if "Windows" in platform.platform():
                                os.rename("%s\\%s" % (json, dirs[i]), "%s\\%s" % (json, ("0%d_" % (i + 1) + dirs[i])))
                            else:
                                os.rename("%s/%s" % (json, dirs[i]), "%s/%s" % (json, ("0%d_" % (i+1)+dirs[i])))
                        if 99 <= i < 999:
                            if "Windows" in platform.platform():
                                os.rename("%s\\%s" % (json, dirs[i]), "%s\\%s" % (json, ("%d_" % (i + 1) + dirs[i])))
                            else:
                                os.rename("%s/%s" % (json, dirs[i]), "%s/%s" % (json, ("%d_" % (i+1)+dirs[i])))
                    if action == 2:
                        ...
                        if "Windows" in platform.platform():
                            os.rename("%s\\%s" % (json, dirs[i]), "%s\\%s" % (json, dirs[i][4:]))
                        else:
                            os.rename("%s/%s" % (json, dirs[i]), "%s/%s" % (json, dirs[i][4:]))
...
if "__main__" == __name__:
    case_number("Cases\\API\\POS\\ALL", 1)
